...
async function seed() {
    ...
    try {
        ...
        const { data: existingSeasons, error: seasonCheckError } = await supabase_1.supabaseAdmin
            .from('seasons')
            .select('id');
        if (seasonCheckError)
            throw seasonCheckError;
        let activeSeasonId = '';
        if (!existingSeasons || existingSeasons.length === 0) {
            console.log('Creating default Season 2026...');
            const { data: newSeason, error: seasonInsertError } = await supabase_1.supabaseAdmin
                .from('seasons')
        ...
        const { data: existingLeader, error: leaderCheckError } = await supabase_1.supabaseAdmin
            .from('users')
            .select('id')
        ...
        if (!existingLeader) {
            ...
            if (authError) {
                ...
                if (authError.message.includes('already registered') || authError.message.includes('already exists')) {
                    ...
                    // Clean up first
                    const { data: usersList } = await supabase_1.supabaseAdmin.auth.admin.listUsers();
                    const existingAuthUser = usersList.users.find(u => u.email === leaderEmail || u.phone === leaderPhone);
                    if (existingAuthUser) {
                        await supabase_1.supabaseAdmin.auth.admin.deleteUser(existingAuthUser.id);
                        // Recreate
                        const { data: newAuthUser, error: retryError } = await supabase_1.supabaseAdmin.auth.admin.createUser({
                            email: leaderEmail,
                            phone: leaderPhone,
                            password: leaderPassword,
                            phone_confirm: true,
